...
const eleIconsContainer = document.getElementById(`icons-container`);

function insertIcons(iconsArray, iconsFakeContainer) {
	eleIconsContainer.innerHTML = ``;

	// creare un ciclo che passa tutte le icone
	iconsArray.forEach((element) => {
		...
		iconsFakeContainer.append(eleDiv);
	});
};

// stampare tutte le icone
...
iconsTypeSelector.addEventListener(`change`, function() {
	...
	if (iconsTypeSelected != "all") {
		...
		// stampare solo l'array di icone filtrate
		insertIcons(filteredIcons, eleIconsContainer);
	}	else {
		// stampare tutte le icone
		insertIcons(arrIcons, eleIconsContainer);
	};
